...
def save_qd_pool(pool, type, u, phase = None):
    ...
    if config.uselocal:
        if config.mixgrids:
            ...
            if phase is not None:
                file_path = './gpdata/QD_pool_' + phase + '_' + str(u)+ '.txt'

            else:
                file_path = './gpdata/QD_pool' + type + str(u) + '.txt'
    else:
        file_path = '/home/user/results/QD_pool'  + phase + '_' + str(u)+ '.txt'
...
def init_everything_else(which_target, maxsize):
    ...
    if not config.fromfile:
        ...
        train_target = Target(which_target,maxsize, 'train', 'targetfromfile/' + which_target)
        test_target = Target(which_target,maxsize, 'test', 'targetfromfile/' + which_target)
...
def eval_and_aggregate_pool_on_subtarget(target, u):
    ...
    with open('gpdata/QD_pool_a_'+str(u)+'.txt', 'rb') as file:
